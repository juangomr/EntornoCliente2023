...
function rellenaUsuario(capaUsuario, usuario) {
  ...
  let calle =
    usuario.location.street.name + "," + usuario.location.street.number;
...
obtenerUsuarios = () => {
  var usu = document.getElementById("usuariosContainer");
  usu.textContent = "Esperando usuarios...";
  ...
  fetch("https://randomuser.me/api/?results=10")
    ...
    .then((data) => {
      ...
      for (let usuario of datos) {
        ...
        usu.appendChild(divUsuario);
      }
    })
    .catch((error) => {
      usu.innerHTML = `<p class='error'>${error}</p>`;
    });
};
